...
function renderScatter(data) {
  ...
  root.append('g').attr('transform', `translate(${m.left},0)`).call(d3.axisLeft(y).tickFormat(d3.format('.0%')))
    .selectAll('text').style('font-size','10px');
  root.append('text').attr('x', w/2).attr('y', h - 8).attr('text-anchor','middle').attr('font-size',11).attr('fill',chrome().muted)
    .text('Coupling A→B (shared / refs in A)');
  root.append('text').attr('x', 14).attr('y', h/2).attr('transform', `rotate(-90, 14, ${h/2})`).attr('text-anchor','middle').attr('font-size',11).attr('fill',chrome().muted)
  ...
  const dots = root.append('g').selectAll('circle').data(pairs).join('circle')
    ...
    .attr('fill', '#5a3e28').attr('opacity', 0.6).attr('stroke', chrome().ink).attr('stroke-width', 0.4)
...
function renderTable(data) {
  ...
  pairs.forEach(p => {
    ...
    const oth = dep === 'a' ? 'b' : 'a';
    html += '<tr>';
    html += `<td style="padding:0.3rem 0.5rem;border-bottom:1px solid var(--chart-grid);"><a href="/article/${p[dep]}" style="color:#5a3e28;">${escapeHtml(p[dep+'_title'] || '#'+p[dep])}</a><div style="font-size:0.78rem;color:var(--chart-muted);">${escapeHtml(p[dep+'_journal'] || '')} · ${p[dep+'_year'] || '—'}</div></td>`;
    html += `<td style="padding:0.3rem 0.5rem;border-bottom:1px solid var(--chart-grid);"><a href="/article/${p[oth]}" style="color:#5a3e28;">${escapeHtml(p[oth+'_title'] || '#'+p[oth])}</a><div style="font-size:0.78rem;color:var(--chart-muted);">${escapeHtml(p[oth+'_journal'] || '')} · ${p[oth+'_year'] || '—'}</div></td>`;
    html += `<td style="padding:0.3rem 0.5rem;border-bottom:1px solid var(--chart-grid);">${p.shared}</td>`;
    html += `<td style="padding:0.3rem 0.5rem;border-bottom:1px solid var(--chart-grid);font-weight:600;">${(p.asymmetry * 100).toFixed(1)}%</td>`;
    html += '</tr>';
  });
  html += '</tbody></table>';
  el.innerHTML = html;
